  ...
  if (first_word == "Bus") {
    ...
    if (info.empty()) {
      ...
      output << std::to_string(info.size()) << " stops on route, "s
             << std::to_string(transport_catalogue.GetUniqueStations(second_word)) << " unique stops, "s
             << std::setprecision(6) << transport_catalogue.FindBus(second_word)->length << " route length\n"s;
